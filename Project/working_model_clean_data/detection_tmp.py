...
def plot_origin_image(image_np, boxes, classes, scores, category_index, prediction_color):

    # Size of the output images.
    IMAGE_SIZE = (12, 8)
    # Change one element in category_index dictionary
    if 10 in category_index:
        category_index[10]['name'] = prediction_color
...
def detect_traffic_lights(PATH_TO_TEST_IMAGES_DIR, MODEL_NAME, Num_images, plot_flag=False,padding=3):
    """
    ...
    with detection_graph.as_default():
        with tf.compat.v1.Session(graph=detection_graph) as sess:
            ...
            for image_path in TEST_IMAGE_PATHS:
                ...
                # cv2.imwrite(os.path.join('./video_processing/temp_after', 'frame_{:04d}.png'.format(index_counter)), cv2.cvtColor(image_np, cv2.COLOR_RGB2BGR))
                index_counter += 1
                
    return commands

def annotate_image(image_path, output_image_path, prediction_color, plot_flag=False):
    image = Image.open(image_path)
    image_np = load_image_into_numpy_array(image)
    image_np_expanded = np.expand_dims(image_np, axis=0)
    ...
    with detection_graph.as_default():
        with tf.compat.v1.Session(graph=detection_graph) as sess:
            ...
            plot_origin_image(image_np, boxes, classes, scores, category_index, prediction_color)
